...
namespace ECCMain
{
    public class Calculate
    {
        ...
        public static BigInteger[] pointPlus(BigInteger[] point_P, BigInteger[] point_Q, out bool isZero)
        {
            ...
            {
                isZero = false;
                var s = (point_P[1] - point_Q[1]) * (ECCMain.Inverse.ex_gcd((point_P[0] - point_Q[0] + Secp256k1.p) % Secp256k1.p, Secp256k1.p));
                s = s % Secp256k1.p;
                if (s < 0)
                {
                    s += Secp256k1.p;
        ...
        private static BigInteger[] getDoubleP(BigInteger[] bigIntegers, out bool isZero)
        {
            ...
            //else
            {
                var s = ((3 * x * x + Secp256k1.a) * (ECCMain.Inverse.ex_gcd((2 * y) % Secp256k1.p, Secp256k1.p))) % Secp256k1.p;
